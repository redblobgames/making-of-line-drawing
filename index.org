...
The labels are overlapping the drag handles. I'll make the drag handles smaller later, when I'm polishing colors, fonts, spacing, etc.
...
#+include: "9/line-drawing.js" src js :lines "26-28"
...
** Zero length lines
:PROPERTIES:
:CUSTOM_ID: zero-length-lines
:END:

The code I originally presented fails when the line distance is zero, e.g. when the start and end points are the same. I had to special case ~N == 0~ in =interpolationPoints=.
...
** SVG transforms
:PROPERTIES:
:CUSTOM_ID: svg-transforms
:END:

You may have noticed that there are a /lot/ of places I multiply by =scale= and add =0.5=. A lot of this would've been simpler if I had picked a different coordinate system. Instead of pixels, I could have chosen a coordinate system where grid x=1.0 would map to (1.0+1.5)*scale = 33. SVG offers two ways to do this, either with =viewBox= or with =transform=. In the original article I used =transform=.
